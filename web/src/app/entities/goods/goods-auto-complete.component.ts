...
export const GOODS_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => GoodsCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-goods pTemplate="item">
                <goods-line [goods]="goods"></goods-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'goods-auto-complete',
    providers: [GOODS_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class GoodsCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Goods = null;

    public suggestions : Goods[] = [];

    //Placeholders for the callbacks
    private _onTouchedCallback: () => void = () => {};
    private _onChangeCallback: (_:any) => void = () => {};

    constructor(private goodsService : GoodsService, private messageService : MessageService) {
    }
    ...
    complete(event:any) {
        this.goodsService.complete(event.query).
            subscribe(
