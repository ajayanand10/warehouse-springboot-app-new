...
export class GoodsListComponent {
    ...
    example : Goods = new Goods();

    // list is paginated
    currentPage : PageResponse<Goods> = new PageResponse<Goods>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.goodsService.getPage(this.example, event).
            subscribe(
    ...
    showDeleteDialog(rowData : any) {
        let goodsToDelete : Goods = <Goods> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(goodsToDelete);
            }
        });
    }

    private delete(goodsToDelete : Goods) {
        let id =  goodsToDelete.id;

        this.goodsService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(goodsToDelete);
