...
export class GuestListComponent {
    ...
    example : Guest = new Guest();

    // list is paginated
    currentPage : PageResponse<Guest> = new PageResponse<Guest>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.guestService.getPage(this.example, event).
            subscribe(
    ...
    showDeleteDialog(rowData : any) {
        let guestToDelete : Guest = <Guest> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(guestToDelete);
            }
        });
    }

    private delete(guestToDelete : Guest) {
        let id =  guestToDelete.id;

        this.guestService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(guestToDelete);
