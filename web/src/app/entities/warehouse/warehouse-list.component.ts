...
export class WarehouseListComponent {
    ...
    example : Warehouse = new Warehouse();

    // list is paginated
    currentPage : PageResponse<Warehouse> = new PageResponse<Warehouse>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.warehouseService.getPage(this.example, event).
            subscribe(
    ...
    showDeleteDialog(rowData : any) {
        let warehouseToDelete : Warehouse = <Warehouse> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(warehouseToDelete);
            }
        });
    }

    private delete(warehouseToDelete : Warehouse) {
        let id =  warehouseToDelete.id;

        this.warehouseService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(warehouseToDelete);
