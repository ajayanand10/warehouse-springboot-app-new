...
export const WAREHOUSE_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => WarehouseCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-warehouse pTemplate="item">
                <warehouse-line [warehouse]="warehouse"></warehouse-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'warehouse-auto-complete',
    providers: [WAREHOUSE_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class WarehouseCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Warehouse = null;

    public suggestions : Warehouse[] = [];
    ...
    complete(event:any) {
        this.warehouseService.complete(event.query).
            subscribe(
