...
export class RateListComponent {
    ...
    example : Rate = new Rate();

    // list is paginated
    currentPage : PageResponse<Rate> = new PageResponse<Rate>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.rateService.getPage(this.example, event).
            subscribe(
    ...
    showDeleteDialog(rowData : any) {
        let rateToDelete : Rate = <Rate> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(rateToDelete);
            }
        });
    }

    private delete(rateToDelete : Rate) {
        let id =  rateToDelete.id;

        this.rateService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(rateToDelete);
