...
public class WarehouseDTOService {
    ...
    public WarehouseDTO save(WarehouseDTO dto) {
        ...
        warehouse.getTheGoods().clear();
        if (dto.theGoods != null) {
            dto.theGoods.stream().forEach(aGood -> warehouse.addAGood(goodsRepository.findOne(aGood.id)));
        }
    ...
    public WarehouseDTO toDTO(Warehouse warehouse, int depth) {
        ...
        if (depth-- > 0) {
            ...
            dto.theGoods = warehouse.getTheGoods().stream().map(aGood -> goodsDTOService.toDTO(aGood, fdepth)).collect(Collectors.toList());
        }
